...
from schemas import BookingOut, ErrorResponse, SEAT_CLASSES, calculate_price
...
def book_flight(db: Session, user_id: int, name: str, flight_id: int, seat_class: str = 'economy') -> BookingOut | ErrorResponse:
    ...
    if seat_class not in SEAT_CLASSES:
        return ErrorResponse(
            ...
            details=f"Seat class '{seat_class}' is not valid. Must be one of: {', '.join(SEAT_CLASSES.keys())}"
        )
    ...
    available_seats = getattr(flight, seat_field)
    
    if available_seats < 1:
        return ErrorResponse(
            error=f"No {seat_class} seats available",
            error_code="NO_SEATS_AVAILABLE",
            details=f"The flight has no available {seat_class} class seats. Please try a different class or another flight."
        )
...
def cancel_booking(db: Session, booking_id: int) -> BookingOut | ErrorResponse:
    ...
    if flight:
        seat_field = f"{booking.seat_class}_seats"
        current_seats = getattr(flight, seat_field)
        setattr(flight, seat_field, current_seats + 1)
